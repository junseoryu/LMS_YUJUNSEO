...
function getStuListView(jsonData){
	...
	stuList.innerHTML="";
	stuList.innerHTML +="<div><div style = 'float: left; width: 50%;'><span name='attend' style='padding-left:30px;' >학생이름</span></div>";
	stuList.innerHTML +="<div style = 'width: 50%; margin-left:150%;'><span name='attend'' >상태</span></div></div>";
	
	for(i=0; i<jsonData.length; i++){
		if(jsonData[i].stuName != null){
			stuList.innerHTML += "<br><div><span>"+jsonData[i].stuName+"&nbsp;&nbsp;&nbsp;&nbsp;</div><div class= 'container'><input type = 'button' class='btn btn-warning' name = 'late', value='"+"지각"+"' style='margin-left:20px; margin-right:20px;' onClick='check(this.value, "+i+")' /><input type = 'button' class='btn btn-primary' name = 'early', value='"+"조퇴"+"' style='margin-left:20px;margin-right:20px;' onClick='check(this.value, "+i+")' /><input type = 'button' class='btn btn-danger' name = 'absent', value='"+"결석"+"' style='margin-left:20px;margin-right:20px;' onClick='check(this.value, "+i+")' /><span name='attend' style='padding-left:30px; font-style: italic;' >출석</span></span></div><br>";
